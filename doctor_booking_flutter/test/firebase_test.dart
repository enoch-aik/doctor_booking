...
bool isSameAppointments(Appointment a, Appointment b) {
  ...
  if (a.bookingStart != null && b.bookingStart != null) {
    isIdentical =
        isIdentical && (a.bookingStart!.isAtSameMomentAs(b.bookingStart!));
  }
  if (a.bookingEnd != null && b.bookingEnd != null) {
    isIdentical =
        isIdentical && (a.bookingEnd!.isAtSameMomentAs(b.bookingEnd!));
  }
  ...
  group('UserData', () {
    group("PatientData", () {
      ...
      test("Check Patient Mail for non-empty database", () async {
        ...
        final mock_data = <String, String>{"name": "John", "surname": "Doe"};
        await instance
            .collection('patients')
            .doc("patient1@bth.se")
            .set(mock_data);
      ...
      test("Check Patient Data Insert", () async {
        ...
        final user = NewUser(
            fullName: "John Doe", emailAddress: email, password: "password");
    ...
    group("DoctorData", () {
      ...
      test("Check Doctor Data Insert", () async {
        ...
        final doctor = NewDoctor(
            fullName: "Jane Doe, M.D.",
            emailAddress: email,
            password: "password",
            speciality: "Optometrist");
        final doctorCred = await auth.createUserWithEmailAndPassword(
    ...
    group("AppointmentTest", () {
      test("Check Appointment Insert Single Patient & Doctor", () async {
        ...
        final auth = MockFirebaseAuth();
        FirebaseApi firebaseApi = FirebaseApi(instance);
        FCMService fcmService = FCMService(serverKey: fcmServerKey);

        final AppointmentDataSourceImpl appointmentDS =
            AppointmentDataSourceImpl(instance, firebaseApi,fcmService);

        // Insert Doctor
        const doctorMail = "doctor1@bth.se";
        final doctorUser = NewDoctor(
            fullName: "Jane Doe, M.D.",
            emailAddress: doctorMail,
            password: "password",
            speciality: "Optometrist");
        final doctorCred = await auth.createUserWithEmailAndPassword(
            email: doctorUser.emailAddress, password: doctorUser.password);
        firebaseApi.storeDoctorData(
            newDoctor: doctorUser, credential: doctorCred);
        Doctor doctor = await firebaseApi.getDoctorData(doctorMail);

        // Insert Patient
        const patientMail = "patient1@bth.se";
        final patientUser = NewUser(
            fullName: "John Doe",
            emailAddress: patientMail,
            password: "password");
        final patientCred = await auth.createUserWithEmailAndPassword(
            email: patientUser.emailAddress, password: patientUser.password);
        firebaseApi.storePatientData(
            newUser: patientUser, credential: patientCred);
        ...
        final appointment = Appointment(
            ...
            bookingEnd: appointmentEnd,
            patientId: patient.userId,
            doctorId: doctor.userId,
            userEmail: patient.emailAddress,
            patientNote: "assessment of visual acuity",
        ...
        final returnV = await appointmentDS.bookDoctorAppointment(
      ...
      test("Check Appointment Correctly Added - Single Doctor & patient",
          () async {
        final instance = FakeFirebaseFirestore();
        final auth = MockFirebaseAuth();
        FirebaseApi firebaseApi = FirebaseApi(instance);
        ...
        // unclear whether any other value can be returned => Todo: test for error
        await appointmentDS.bookDoctorAppointment(
            newAppointment: appointment,
            doctor: doctor,
            patientEmail: patient.emailAddress);
        bool valid = true;
        final patientAppointments =
            (await firebaseApi.getPatientData(patientMail)).appointments;
        final doctorAppointments =
            (await firebaseApi.getDoctorData(doctorMail)).appointments;
        // appointment might profit from a unique id.
        //print(patientAppointments.map((e) => e.hashCode).first)
        valid = valid &&
            patientAppointments
                .map((e) => isSameAppointments(e, appointment))
                .contains(true);
        valid = valid &&
            doctorAppointments
                .map((e) => isSameAppointments(e, appointment))
      ...
      test(
          "Check Appointment Correctly Added - Single Doctor & Multiple patients - No Doctor reloading",
          () async {
        final instance = FakeFirebaseFirestore();
        final auth = MockFirebaseAuth();
        FirebaseApi firebaseApi = FirebaseApi(instance);
        ...
        for (int i = 0; i < 5; i++) {
          ...
          var user = NewUser(
              fullName: name, emailAddress: patientMail, password: name);
          final userCred = await auth.createUserWithEmailAndPassword(
              email: user.emailAddress, password: user.password);
          firebaseApi.storePatientData(newUser: user, credential: userCred);
          Patient patient = await firebaseApi.getPatientData(patientMail);
      ...
      test(
          "Check Appointment Correctly Added - Single Doctor & Multiple patients",
          () async {
        final instance = FakeFirebaseFirestore();
        final auth = MockFirebaseAuth();
        FirebaseApi firebaseApi = FirebaseApi(instance);
